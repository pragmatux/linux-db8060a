...
* Version	: V 1.0.9
* Date		: 2011/May/23
* Description	: LSM303DLHC 6D module sensor device driver
*
********************************************************************************
*
...
struct sensor_regulator lsm303dlhc_mag_vreg[] = {
	{NULL, "vdd2v9", 2800000, 3300000},
	{NULL, "vdd1v8", 1800000, 200000},
};

static struct lsm303dlhc_mag_data *__mag;
...
static int lsm303dlhc_mag_i2c_write(struct lsm303dlhc_mag_data *mag,
				   u8 *buf, int len)
{
	int err;
...
static ssize_t attr_get_enable(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct lsm303dlhc_mag_data *mag = __mag;
...
static int lsm303dlhc_mag_input_init(struct lsm303dlhc_mag_data *mag)
{
	...
	if (device_create_file(&mag->input_poll_dev->input->dev,
				&dev_attr_enable) < 0) {
...
{
	device_remove_file(&mag->input_poll_dev->input->dev,&dev_attr_enable);
			   ...
			   const struct i2c_device_id *id)
{
	...
	err = create_sysfs_interfaces(mag->dev);
	if (err < 0) {
...
{
	pr_err(">>>lsm303dlhc magnetometer driver<<<\n");
